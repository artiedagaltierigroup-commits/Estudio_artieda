...
import { voidSavingsContributionFromExpense } from "@/actions/savings";
...
import { formatDate, formatDateTime, getExpenseOriginLabel, getExpenseTypeLabel } from "@/lib/utils";
...
export default async function GastoDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleVoidSavingsContribution(formData: FormData) {
    "use server";
    await voidSavingsContributionFromExpense(id, String(formData.get("reason") ?? ""));
    redirect(`/gastos/${id}`);
  }

  const isVoided = Boolean(expense.voidedAt);
  const isSavingsExpense = expense.origin === "SAVINGS";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]">
        ...
        >
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-[24px] border border-border/70 bg-white/80 p-5">
              <p className="text-[0.72rem] font-semibold uppercase tracking-[0.16em] text-muted-foreground">Origen</p>
              <p className="mt-2 text-sm font-medium text-foreground">{getExpenseOriginLabel(expense.origin)}</p>
            </div>
            <div className="rounded-[24px] border border-border/70 bg-white/80 p-5">
        ...
        <SectionCard
          eyebrow="Anulacion"
          title={isSavingsExpense ? "Anular aporte" : "Anular gasto"}
          description={
            isSavingsExpense
              ? "Anula el gasto y tambien descuenta el aporte de la meta de ahorro."
              : "Si el egreso fue cargado por error, conviene anularlo en vez de perder el rastro."
          }
        >
          {isSavingsExpense && isVoided ? (
            <div className="rounded-[24px] border border-dashed border-border/80 bg-muted/20 p-5 text-sm text-muted-foreground">
              Este aporte de ahorro fue anulado el {expense.voidedAt ? formatDateTime(expense.voidedAt) : "-"}.
              {expense.voidReason ? ` Motivo: ${expense.voidReason}` : ""}
            </div>
          ) : isSavingsExpense ? (
            <form action={handleVoidSavingsContribution} className="space-y-4">
              <div className="rounded-[24px] border border-dashed border-border/80 bg-muted/20 p-5 text-sm leading-6 text-muted-foreground">
                Esta accion anula el gasto automatico y descuenta el aporte del progreso del ahorro. Para corregir un monto,
                anulalo aca y carga el aporte correcto desde Ahorros.
              </div>
